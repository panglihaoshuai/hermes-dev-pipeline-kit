...
## Evidence Ownership (v0.3)

For M/L tasks, the preferred final report is generated by `scripts/final-report.sh` from `generated/run-state.json`.

- Agent writes raw evidence.
- Harness generates run-state.
- Policy-check validates generated run-state.
- Final report is generated from run-state.

Do not treat a hand-written run-state or free-form agent report as runtime behavior evidence. It may be used as a policy fixture only.
