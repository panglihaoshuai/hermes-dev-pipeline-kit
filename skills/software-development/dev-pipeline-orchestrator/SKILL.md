...
## v0.3 Executable Evidence Harness Runtime

For M/L tasks, Hermes must use the v0.3 executable evidence harness at runtime:

- Before implementation starts, call `run-init.sh` to create the run directory and initial manifest/classification/work-order files.
- All verification commands must be executed through `record-command.sh` so command, cwd, timestamps, stdout, stderr, and exit code are captured.
- Final run-state must be generated by `generate-run-state.sh`.
- Final owner report must be generated by `final-report.sh`.
- Agent must not hand-write final run-state.
- If `run-init.sh` or `record-command.sh` cannot be found, M/L tasks must stop and fail closed.

Harness script discovery order:
1. installed skill local scripts/bin path;
2. `$HERMES_DEV_PIPELINE_KIT_ROOT/scripts`;
3. current repository `scripts/`;
4. if not found, fail closed.

Required installed script names:
