...
- `skill_trace.display_language=zh-CN`
- 但缺少 `skill_trace.current_phase_label`
...
```json
{
  "skill_trace": {
    ...
    "current_phase_label": "方案设计与计划编写",
    "current_phase_internal": "planning",
    "user_visible_skill_banner": true
  }
}
```

如果曾向用户提出 blocking questions，还需要：

```json
{
  "clarification_trace": {
    "stage_label": "需求澄清与关键问题确认",
    "skills_used": ["dev-pipeline-orchestrator", "Simple Prompt Intake", "writing-plans"],
    "why_questions_are_needed": "这些问题会影响架构拆分和工单范围。",
    "next_stage_label": "方案设计与计划编写"
  }
}
```

### 原则
用户交互中不能只显示英文 phase，例如 `Phase: planning`。中文阶段名必须是主显示文本，英文内部名只能作为括号内 trace。
...
### evidence-blocking-acceptance FAIL
**Symptom**: acceptance.complete=true but Matt evidence is blocking and missing
**Fix**: Set acceptance.complete=false or provide Matt evidence

### codex-deferred-consistency FAIL
**Symptom**: Codex deferred but status_color=green
**Fix**: Set status_color=yellow or red

### self-improvement-side-effect FAIL
**Symptom**: Self-improvement triggered without user approval
**Fix**: Get user approval or write to backlog only

### tdd-red-evidence FAIL
**Symptom**: required_matt_skill=tdd but no RED evidence
**Fix**: Add RED evidence or red_not_applicable_reason

### provenance FAIL
**Symptom**: M/L run-state is missing provenance or was not generated by `scripts/generate-run-state.sh`
**Fix**: Create a run directory with `scripts/run-init.sh`, record commands with `scripts/record-command.sh`, then run `scripts/generate-run-state.sh <run-dir>`

### claudecode-result-contract FAIL
**Symptom**: ClaudeCode result contains `acceptance.complete`
**Fix**: Remove acceptance from `raw/claudecode-result.json`; only the harness may generate final acceptance state

### tdd-command-log-evidence FAIL
**Symptom**: TDD evidence is text-only or missing RED/GREEN command-log exit codes
**Fix**: Use `scripts/record-command.sh` for RED and GREEN commands so `raw/command-log.jsonl` proves the sequence
