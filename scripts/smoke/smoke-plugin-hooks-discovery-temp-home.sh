#!/usr/bin/env bash
# smoke-plugin-hooks-discovery-temp-home.sh — verify hook prototype plugin discovery using temp HOME.
set -euo pipefail

REPO_ROOT="$(cd "$(dirname "$0")/../.." && pwd)"
PLUGIN_SRC="$REPO_ROOT/plugins/hermes-evidence-runtime"
PLUGIN_NAME="hermes-evidence-runtime"
TMP_HOME="/tmp/hermes-plugin-hooks-discovery-home"
OUTPUT_PATH="/tmp/hermes-plugin-hooks-discovery-output.txt"
REAL_HOME="${HOME:-}"
...
plugin_copied="no"
hooks_source_copied="no"
if [[ -f "$TMP_HOME/.hermes/plugins/$PLUGIN_NAME/plugin.yaml" ]]; then
  plugin_copied="yes"
fi
if [[ -f "$TMP_HOME/.hermes/plugins/$PLUGIN_NAME/hooks.py" ]]; then
  hooks_source_copied="yes"
fi
...
hook_listing="UNKNOWN"
if grep -Eq 'pre_tool_call|post_tool_call|on_session_end|on_session_finalize|subagent_stop' "$OUTPUT_PATH"; then
  hook_listing="yes"
fi

verdict="FAIL"
if [[ "$list_exit" -eq 0 && "$plugin_copied" == "yes" && "$hooks_source_copied" == "yes" && "$plugin_discovered" == "yes" && "$real_home_touched" == "no" ]]; then
  verdict="PASS"
fi
...
echo "hook listing available: $hook_listing"
echo "output path: $OUTPUT_PATH"
...
echo "smoke-plugin-hooks-discovery-temp-home: PASS"
