#!/usr/bin/env bash
# smoke-worker-normalizer-to-run-state.sh — Verify normalized worker result flows into run-state.
set -euo pipefail

REPO_ROOT="$(cd "$(dirname "$0")/../.." && pwd)"
TMP_ROOT="$(mktemp -d /tmp/hermes-worker-normalizer-run-state.XXXXXX)"
...
SIM_DIR="$TMP_ROOT/simulated-worker"
mkdir -p "$WORK_DIR" "$SIM_DIR"

TASK_FILE="$TMP_ROOT/task.md"
cat > "$TASK_FILE" <<'EOF'
Implement a tiny todo store and normalize simulated worker output into run-state evidence.
EOF
...
RUN_DIR="$(bash "$REPO_ROOT/scripts/run-init.sh" \
  ...
  --mode auto_run \
  --task-type feature \
  --project worker-normalizer-smoke)"
...
store.add("write normalizer smoke");
if (store.list().length !== 1) throw new Error("expected one item");
if (store.list()[0].title !== "write normalizer smoke") throw new Error("bad title");
console.log("todo normalizer smoke pass");
EOF
...
bash "$REPO_ROOT/scripts/simulate-worker-output.sh" \
  --worker claude-code \
  --out-dir "$SIM_DIR" >/dev/null

NORMALIZED_WORKER_RESULT="$TMP_ROOT/worker-result.json"
bash "$REPO_ROOT/scripts/normalize-worker-result.sh" \
  --worker claude-code \
  --worker-skill simulated/mattpocock-tdd \
  --work-order-id WO-1 \
  --status completed \
  --result-type implementation \
  --raw-output "$SIM_DIR/raw.txt" \
  --structured-output "$SIM_DIR/structured.json" \
...
bash "$REPO_ROOT/scripts/record-worker-result.sh" \
  ...
  --raw-output "$SIM_DIR/raw.txt" >/dev/null
...
assert state["worker_results"][0]["worker"] == "claude-code"
...
echo "smoke-worker-normalizer-to-run-state: PASS"
